...
int main (int argc, char *argv[])
{
  ...
  while (TRUE) {
    ...
    if (strlen (data) == 0)
      continue;
    ki = malloc (TEST_DATA_SIZE + sizeof (uint32));
    if (0 == ki) {
      printf ("\n\n***Error: fatal memory error allocating a keyItem\n");
      return _ERROR_;
    }
    /*memset (ki, 0, TEST_DATA_SIZE + sizeof (uint32)); */
    check_pointer (ki);
...
/*	strcpy (idx->hdr->indexName, "testIndex.idx");*/
  ...
  while (TRUE) {
    ...
    if (0 == tmp) {
      printf ("\n\n***Error: deleting key = %s, item = %d, counter = %d: %s\n", 
  ...
  free(rm);
  printf ("Deleted every index item.  Indexes remaining = %d\n(should be 0)\n", 
      idx->hdr->numItems);
