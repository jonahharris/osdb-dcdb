...
int bsort_num_compares = 0;

INLINE int qsort_testCompare (void *p1, void *p2)
{
  bsort_num_compares++;
        return (strcmp ((char *)p1, (char *)p2));
}
...
int main (int argc, char *argv[])
{
  ...
  /* just print the data values. */
  for (i = 0; pdata[i] && i < MAXDATA; i++)
    printf ("%s\n", pdata[i]);
  fprintf (stderr, "Added in %8.6f seconds\n", t2-t1);
  ...
  bqsort ((void*)qsortarray, number, DATASIZE,
      (int(*)(const void*, const void*))qsort_testCompare);
  elapsed(&t2);
  fprintf (stderr, "Added with my qsort in %8.6f seconds\n", t2-t1);
  ...
  elapsed (&t2);
  fprintf (stderr, "Added with system qsort in %8.6f seconds\n", t2-t1);
