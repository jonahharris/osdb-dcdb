...
int main (int argc, char *argv[])
{
   ...
   * Populate the shell
   */
  shl = initShell (testCompare, TRUE, TRUE);
#ifndef __CINT__
  elapsed (&t1);
#else
  t1 = dbtime ();
...
/*#ifdef MASS_DUPS
    ...
    if (status == _ERROR_) {
      if (shl->shlError == SHELL_UNIQUE) {
	set_shlError(shl, SHELL_NOERR);
	continue;
      }
	...
	printf ("\n\n***Error: item %d, %s entering \"%s\"\n",
		counter, shlErrorStr[shl->shlError], ln);
	delShell (shl, 0);
	return _ERROR_;
      }
    }
    debugCheckShell (shl);
    if (shl->shlError == SHELL_UNSPECIFIED)
      return _ERROR_;
  ...
  printf ("\nNumber of restructures during adds: %d\n\n",
      shl->numRestruct);
