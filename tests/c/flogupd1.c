...
int main (void)
{
  ...
  if (fexists ("flogupd1.db")) {
    fileRemove ("flogupd1.db");
    fileRemove ("flogupd1.db.LCK");
    fileRemove ("fu1_ssnidx.idx");
    fileRemove ("fu1_ssnidx.inx");
    fileRemove ("fu1_lnidx.idx");
    fileRemove ("fu1_lnidx.inx");
  }
  fp = fopen ("flogupd1.df", "wb");
  if (0 == fp) {
    printf ("\n\n***Error: could not open flogupd1.df to write\n");
    return _ERROR_;
  }
  fprintf (fp,
      "//flogupd1.df\n\n"
      "create table \"flogupd1.db\"\n"
      "  info \"Table for flogupd1.c\"\n"
      "{\n"
      "  \"LName\" char (65);\n"
      "  \"SSNumber\" char (9);\n"
      "  \"FName\" char (35);\n"
      ...
      "  \"LastUpdated\" date;\n"
      "} indexed {\n"
      "  idx \"fu1_ssnidx\" 128:case:unique \"SSNumber\";\n"
      "  idx \"fu1_lnidx\" 512:case:dup \"LName\";\n"
      "};\n\n"
      );
  fclose (fp);

  status = dbcreate ("flogupd1.df");
  if (status == _ERROR_) {
    printf ("\n\n***Error: dbcreate(%s): %s\n", "flogupd1.df", cdberror);
    return _ERROR_;
  }

  field = (char*)dbopen ("flogupd1.db");
  if (field == 0) {
    printf ("\n\n***Error: dbopen (%s): %s\n", "flogupd1.db", cdberror);
    return _ERROR_;
  }
  strcpy (table, field);
  ...
  fp = fopen ("testAdd2.txt", "rb");
  if (0 == fp) {
    printf ("\n\n***Error: could not open testAdd2.txt to read\n");
    dbexit ();
    return _ERROR_;
  }
  t1 = dbtime ();
  ...
  for (i = 0; i < MAXRECORDS; i++) {
     ...
     * Update it.
     */
    status = dbupdate (table);
    if (status != _OK_) {
      if (! strcmp (cdberror,
	    "updating record: index: unique constraint violated")) {
        printf ("\n\n***Warning: unique constraint violated in update\n");
	dbclose (table);
	status = dbreindex ("flogupd1.db");
	if (status == _ERROR_) {
	  printf ("\n\n***Error: dbreindex (flogupd1.db): %s\n", cdberror);
	  dbexit();
	  return _ERROR_;
	}
	else {
	  printf ("flogupd1.db should be OK - reindex was successful\n");
	  return 0;
        }
      }
    }
  ...
  t2 = dbtime ();
  numrecs = dbnumrecs (table);
  printf ("\nFinished updating %d records in %6.4f seconds\n", numrecs, t2-t1);
  dbexit ();
  return 0;
}
