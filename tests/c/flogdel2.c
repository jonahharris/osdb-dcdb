...
int main (int argc, char *argv[])
{
  ...
  if (fexists ("flogdel2.db")) {
    fileRemove ("flogdel2.db");
    fileRemove ("flogdel2.db.LCK");
    fileRemove ("fd2_ssnidx.idx");
    fileRemove ("fd2_ssnidx.inx");
    fileRemove ("fd2_lnidx.idx");
    fileRemove ("fd2_lnidx.inx");
  }
  fp = fopen ("flogdel2.df", "wb");
  if (0 == fp) {
    printf ("\n\n***Error: could not open flogdel2.df to write\n");
    return _ERROR_;
  }
  fprintf (fp,
      "//flogdel2.df\n\n"
      "create table \"flogdel2.db\"\n"
      "  info \"Table for flogdel2.c\"\n"
      "{\n"
      "  \"LName\" char (65);\n"
      "  \"SSNumber\" char (9);\n"
      "  \"FName\" char (35);\n"
      ...
      "  \"LastUpdated\" date;\n"
      "} indexed {\n"
      "  idx \"fd2_ssnidx\" 128:case:unique \"SSNumber\";\n"
      "  idx \"fd2_lnidx\" 512:case:dup \"LName\";\n"
      "};\n\n"
      );
  fclose (fp);

  status = dbcreate ("flogdel2.df");
  if (status == _ERROR_) {
    printf ("\n\n***Error: dbcreate(%s): %s\n", "flogdel2.df", cdberror);
    return _ERROR_;
  }

  field = (char*)dbopen ("flogdel2.db");
  if (field == 0) {
    printf ("\n\n***Error: dbopen (%s): %s\n", "flogdel2.db", cdberror);
    return _ERROR_;
  }
  strcpy (table, field);
