...
/*
 * I pulled this into DCDB to provide a 32-bit CRC function for my programs.
 * I added some headers to the include list and added the crc32Sum() function.
 */
...
unsigned crc32Sum (char *fname)
{
  ...
  if (0 == fp) {
    if (EACCES == errno)
      dbError = DB_IO_DENIED;
    else if (EMFILE == errno)
      dbError = DB_IO_TOOMANY;
    else if (ENOENT == errno)
      dbError = DB_IO_NOFILE;
    else
